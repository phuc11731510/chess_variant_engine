...
echo "[colab] Full AlphaZero loop on GPU:"
echo "  python $PY_DIR/loop.py --engine $BIN --gens 10 --games-per-gen 1000 --visits 200 \\"
echo "      --window-gens 4 --epochs 20 --batch 1024 --provider cuda --fixed-batch 32 \\"
echo "      --parallel 2 --eval-games 40 --diff-focus --workdir $PY_DIR/loop_run"
