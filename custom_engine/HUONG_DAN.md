...
**DÙNG ĐƯỢC — chỉnh sâu kiểu lc0** (đặt bằng tên gốc, vd `setoption name cpuct value 2.5`):
`cpuct` (1.745) · `cpuct-base` (38739) · `cpuct-factor` (3.894) · `fpu-value` (0.330) ·
`fpu-strategy` (reduction/absolute) · `draw-score` (0, thiên về/né hòa) · `two-fold-draws` (false) ·
`temp-decay-moves` · `temp-cutoff-move` · `temp-endgame` · `policy-softmax-temp` (1.359).
*(Mặc định các tham số tìm kiếm = đúng default của lc0-master.)*
...
**CHƯA DÙNG ĐƯỢC (chạy ở default lc0, không có nút):** ~60 tham số search lc0 còn lại (cpuct-at-root,
max-collision-*, fpu-*-at-root, nps-limit, wdl-calibration…); `go depth N`/`go mate N` (nhận nhưng chưa
giới hạn theo depth/mate); `score mate N` (chưa phát); ponder mới ở mức cơ bản (kết thúc khi `ponderhit`,
chưa cấp thêm ngân sách thời gian).
...
**CHƯA DÙNG ĐƯỢC từ CLI self-play (chạy ở default lc0):** các search-param khác ngoài `--cpuct`
(cpuct-base/factor, fpu-*, draw-score, two-fold-draws, lịch hạ nhiệt temp-decay…) — **search vẫn tôn
trọng chúng**, chỉ là chưa có cờ để đặt (cần `--search-opt k=v`, sẽ thêm sau). `--resign-earliest-move`
/ `--resign-wdlstyle` (resign nâng cao) chưa viết. *Lưu ý: self-play VỐN đã tái dùng cây trong một ván
(không cần cờ riêng).*
...
**CHƯA DÙNG ĐƯỢC (chưa có cờ):** `--accum-steps` (gradient accumulation), `--loss-scale`, `--dropout`,
`--batch-renorm`, `--se-ratio`, `--ema`, `--swa-max-n/--swa-every`, `--max-steps`, `--val-split/--test-every`,
`--report-every`, `--save-every`, `--shuffle-size`, `--max-records`.
