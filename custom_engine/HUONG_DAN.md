...
3. **Chép sang ổ local của Colab rồi build** (build trên Drive RẤT chậm + hay lỗi do FUSE):
   ...
   **3b. Lưu engine đã build lên Drive để phiên sau KHỎI build lại** (đỡ vài phút biên dịch + tải ORT):
   ```
   # CHẠY MỘT LẦN, ngay sau khi colab_setup.sh build xong:
   !bash /content/FairyZero/scripts/colab_prebuilt.sh save
   ```
   Lệnh này chép `custom_engine` + thư viện ONNX Runtime vào `MyDrive/FairyZero_prebuilt`.
   **Các phiên Colab SAU**, thay vì chạy `colab_setup.sh`, chỉ cần khôi phục (vài giây):
   ```
   !cp -r /content/drive/MyDrive/FairyZero /content/FairyZero      # vẫn cần mã nguồn + python/
   !bash /content/FairyZero/scripts/colab_prebuilt.sh restore
   ```
   `restore` chép engine từ cache về, tạo **wrapper `run.sh`** (tự đặt `LD_LIBRARY_PATH`), rồi tự kiểm tra
   `--test-uci`. Từ đó **chạy engine qua wrapper** (vì mỗi ô `!` của Colab là shell mới, biến môi trường không
   sống sót qua ô khác):
   ```
   ...
   ```
   > **Khi nào vẫn phải build lại** (hiếm — `restore` sẽ tự báo và thoát lỗi): Colab đổi ảnh nền
   > (glibc/Ubuntu) ⇒ binary không chạy; hoặc Colab đổi **CUDA major** ⇒ EP CUDA không nạp được (CPU vẫn chạy).
   > Khi đó chạy `colab_setup.sh` một lần rồi `colab_prebuilt.sh save` lại. Cache nằm ở Drive nên đổi `FZ_CACHE=...`
   > để chọn nơi khác; đổi ORT bằng `ORT_VER=1.18.0 bash ... save`.
